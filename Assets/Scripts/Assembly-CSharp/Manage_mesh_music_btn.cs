...
public class Manage_mesh_music_btn : MonoBehaviour
{
	public Mesh p_on_on;
	...
	private IEnumerator SetStartTexture()
	{
		...
		is_on = PlayerPreperencesManager.GetMusic();
		if (is_on)
		{
	...
	private void Update()
	{
		...
		if (Input.GetTouch(0).phase == TouchPhase.Ended)
		{
			...
			{
				base.gameObject.GetComponent<MeshFilter>().mesh = p_on_rel;
				PlayerPreperencesManager.TurnMusicOn();
			}
			else
			{
				base.gameObject.GetComponent<MeshFilter>().mesh = p_off_rel;
				PlayerPreperencesManager.TurnMusicOff();
			}
		}
	}
